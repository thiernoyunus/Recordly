		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			try {
				...
				if (
					...
					const settingsLabel = app.isPackaged ? app.getName() || "Recordly" : "Electron";
					const { response } = await dialog.showMessageBox({
						type: "warning",
						title: "Screen Recording Permission Required",
						message:
							"Recordly needs screen recording permission to capture your screen.",
						detail: `Open System Settings → Privacy & Security → Screen Recording (or "Screen & System Audio Recording"), turn ON "${settingsLabel}", then fully quit and reopen the app.\n\nIf you do not see it in the list yet, click Open System Settings, wait a moment, and look again — the first permission request adds the entry.`,
					...
					if (response === 0) {
						try {
							const { openMacPrivacySettings } = await import("../utils");
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					...
					const { response } = await dialog.showMessageBox({
						...
						detail: `Open System Settings → Privacy & Security → Microphone, turn ON "${settingsLabel}", then try again.\n\nDevelopment builds appear as "Electron", not "Recordly".`,
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
